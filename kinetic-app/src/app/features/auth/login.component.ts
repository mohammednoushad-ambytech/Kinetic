    ...
    <div class="min-h-screen bg-surface flex items-center justify-center p-6">
      <div class="w-full max-w-[440px] flex flex-col items-center">
        ...
        <header class="mb-10 text-center">
          ...
          <h1 class="text-2xl font-bold tracking-tight text-on-surface">Kinetic</h1>
          <p class="text-on-surface-variant text-sm mt-1">Sign in to your editorial workspace</p>
        </header>

        <!-- Card -->
        <main class="w-full bg-surface-container-lowest rounded-xl p-8 ambient-lift ghost-border">
          <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-6">
            ...
            @if (errorMsg()) {
              <div class="p-4 rounded-lg bg-error-container/50 flex items-start gap-3">
                ...
                <div>
                  <p class="text-on-error-container text-[10px] font-bold uppercase tracking-wider">Invalid Credentials</p>
        ...
        <footer class="mt-8 flex flex-col items-center gap-4">
          <nav class="flex gap-6 opacity-60">
            <span class="text-[10px] font-medium text-on-surface tracking-wider">KINETIC v1.0</span>
          </nav>
        </footer>
      </div>
    </div>
